...
from . import tracker
from .conf import get_position, get_show_bar, get_show_headers, get_thresholds, get_enable_console
...
class DevBarMiddleware:
    ...
    def _determine_level(self, stats, total_time, thresholds):
        if total_time > thresholds['time_critical'] or stats['count'] > thresholds['count_critical']:
            return "crit"
        if (stats['has_duplicates'] or
            total_time > thresholds['time_warning'] or
            stats['count'] > thresholds['count_warning']):
            return "warn"
        return "ok"
    ...
    def _inject_devbar(self, response, stats, level):
        ...
        dup_marker = ' <strong style="color:#f59e0b">(D)</strong>' if stats["has_duplicates"] else ""

        css = STYLE_BLOCK % get_position()
        html = BAR_TEMPLATE % (level, stats['duration'], stats['python_time'], stats['count'], dup_marker)
